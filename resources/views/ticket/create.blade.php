...
@section('content')
    <div class="w-75 d-block mx-auto mt-3 p-4">
        ...
        <form method="POST" action="{{ route('dashboard.ticket.store') }}" enctype="multipart/form-data">
            @csrf
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <textarea name="description" type="text" class="form-control @error('description') is-invalid @enderror" rows="5">{{ old('description') }}</textarea>
                @error('description')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
